...
Elevator.prototype = {
	...
	,travelto: function(floors) {
		...
		var floors_to_travel = Std["int"](Math.abs(js.Boot.__cast(floors , Float)));
...
Main.prototype = {
	ready: function() {
		...
		if(StringTools.startsWith(result,"+")) {
			...
			var floors1 = Std.parseInt(result);
			if(floors1 != null) this.elevator.travelto(floors1);
	...
	,spinner: function() {
		var spinnerString = "\r";
		if(this.spinnerState > 6) this.spinnerState = 0;
		var _g = this.spinnerState;
		switch(_g) {
		case 0:
			spinnerString += "-";
			break;
	...
	,checkReady: function() {
		js.Node.process.stdout.write(this.spinner());
		if(!this.elevator.isTravelling) {
			js.Node.process.stdout.write("\r");
			this.ready();
		}
	}
	,__class__: Main
